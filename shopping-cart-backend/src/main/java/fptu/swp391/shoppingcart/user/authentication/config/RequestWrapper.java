    ...
    private void overwriteSetCookie(HttpServletResponse response) {
        if (response != null) {
            ...
            for (String header : headers) { // there can be multiple Set-Cookie attributes
                if (firstHeader) {
                    response.setHeader(HttpHeaders.SET_COOKIE, String.format("%s; %s; Secure", header, "SameSite=None")); // set
                    firstHeader = false;
                    continue;
                }
                response.addHeader(HttpHeaders.SET_COOKIE, String.format("%s; %s; Secure", header, "SameSite=None")); // add
            }
        }
    }
}
